...
class MyWin(wx.Frame):
    def __init__(self, parent, title):
        ...
        self.TopicTag()
        self.notebook.InsertPage(0, self.topicpanel, text = 'Topics and Blocks')
        self.WorkStation()
        self.SetLocation()
        self.Go()
    ...
    def SetTopBar(self):
        ...
        txt = wx.StaticText(self.panel, -1, 'Enter an event\'s name:')
    ...
    def NewsTag(self):
        ...
        text2 = wx.StaticBox(self.newspanel, label = 'The Content')
    ...
    def TopicTag(self):
        ...
        self.blobtn = wx.Button(self.topicpanel, label = 'Show Blocks', style = wx.BU_EXACTFIT)
        ...
        for i in range(0, self.maxblockcnt):
            ...
            tmpbtn = wx.Button(self.blockpanel, 200+i,label = 'Jump to news', style = wx.BU_EXACTFIT)
            tmpbtn.Bind(wx.EVT_BUTTON, self.Jump2News)
            ttbox = wx.BoxSizer(wx.VERTICAL)
            ttbox.Add(tmpcheck)
            ttbox.Add((20, 40))
            ttbox.Add(tmpbtn)
            tmptext = wx.TextCtrl(self.blockpanel, -1, size = (270, 150), style = wx.TE_MULTILINE)
    ...
    def WorkStation(self):
        #最右侧的综述栏
        #上面的自动生成的、不可编辑的综述
        text1 = wx.StaticBox(self.panel, -1, 'Auto-generated')
        self.box41 = wx.StaticBoxSizer(text1, wx.VERTICAL)
        self.solidsum = wx.TextCtrl(self.panel, style = wx.TE_MULTILINE|wx.TE_READONLY|wx.TE_RICH)
        self.solidsum.SetBackgroundColour((240, 240, 240, 255)) #设置颜色为传统的灰色，直观上给人只读的感受
        self.box41.Add(self.solidsum, 1, wx.ALL|wx.EXPAND)

        text2 = wx.StaticBox(self.panel, -1, 'Edit here')
        self.box42 = wx.StaticBoxSizer(text2, wx.VERTICAL)
        self.editsum = wx.TextCtrl(self.panel, style = wx.TE_MULTILINE|wx.TE_RICH)
        self.box42.Add(self.editsum, 1, wx.ALL|wx.EXPAND)

        self.box4 = wx.BoxSizer(wx.VERTICAL)
        self.box4.Add(self.box41, 1, wx.EXPAND|wx.ALL)
        self.box4.Add(self.box42, 1, wx.EXPAND|wx.ALL)
    ...
    def ShowBlock(self, e):
        ...
        self.AddStat(('label %s has %d blocks totally.')%(self.topics[curid].label, b))
    ...
    def Jump2News(self, e):
        ...
        self.notebook.SetSelection(1) #跳到News页
    ...
    def summary(self):
        ...
        for tid in selectTopicid:
            ...
            if wordcnt < std - 40:
                #字数不够啊，报错
                self.AddStat(('Error in generating synthesis of %s:# of word is too big or the choosed block is too little')%(curtopic.label))
                continue
