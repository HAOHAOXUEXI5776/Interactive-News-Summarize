...
class MyWin(wx.Frame):
    def __init__(self, parent, title):
        ...
        #颜色用于标签-综述的关系
        self.color = ['#FF0000', '#02B232', '#0000FF', '#008080', '#FF00FF',
                      '#808080', '#400000', '#800000', '#202020', '#000080',
                      '#400040', '#800080', '#808000', '#804000', '#00FFFF',
        ...
        self.NewsTag()
        self.TopicTag()
        self.notebook.InsertPage(0, self.topicpanel, text = '子话题和文本块')
        self.notebook.InsertPage(0, self.newspanel, text = '新闻文件')
    ...
    def SetTopBar(self):
        ...
        txt = wx.StaticText(self.panel, -1, '输入话题:')
        ...
        self.quick = wx.Button(self.panel, -1, '快捷键', style = wx.BU_EXACTFIT)
        self.Bind(wx.EVT_BUTTON, self.OnQuick, self.quick)
        self.save = wx.Button(self.panel, wx.ID_SAVE, '保存综述', style = wx.BU_EXACTFIT)
        self.Bind(wx.EVT_BUTTON, self.OnSave, self.save)
        self.refresh = wx.Button(self.panel, -1, '清空屏幕', style = wx.BU_EXACTFIT)
        self.Bind(wx.EVT_BUTTON, self.OnRefresh, self.refresh)
        self.about = wx.Button(self.panel, wx.ID_ABOUT, '关于', style = wx.BU_EXACTFIT)
        self.Bind(wx.EVT_BUTTON, self.OnAbout, self.about)
        self.exit = wx.Button(self.panel, wx.ID_EXIT, '退出', style = wx.BU_EXACTFIT)
    ...
    def NewsTag(self):
        ...
        statbox = wx.StaticBox(self.newspanel, -1, '状态')
        ...
        text2 = wx.StaticBox(self.newspanel, label = '新闻内容')
    ...
    def TopicTag(self):
        ...
        #1.标签栏
        #全选，反选框
        self.seleteall = wx.CheckBox(self.topicpanel, label = '全选/全不选')
        self.seleteinv = wx.Button(self.topicpanel, label = '反选', size = (60,23))
        ...
        self.blobtn = wx.Button(self.topicpanel, label = '查看文本块', style = wx.BU_EXACTFIT)
        self.blobtn.Bind(wx.EVT_BUTTON, self.OnBlobtn)
        #综述按钮
        self.sumbtn = wx.Button(self.topicpanel, label = '合成综述', style = wx.BU_EXACTFIT)
        ...
        statbox = wx.StaticBox(self.topicpanel, -1, '状态')
        ...
        #将上述部件放入一个Box内
        staticbox1 = wx.StaticBox(self.topicpanel, -1, '设置子话题')
        ...
        self.bsall = wx.CheckBox(self.btnpanel, -1, '全选/全不选')
        self.bsinv = wx.Button(self.btnpanel, -1, '反选', size = (60, 23), style = wx.BU_EXACTFIT)
        ...
        for i in range(0, self.maxblockcnt):
            ...
            tmpbtn = wx.Button(self.blockpanel, 200+i, label = '查看原文', style = wx.BU_EXACTFIT)
        ...
        staticbox2 = wx.StaticBox(self.topicpanel, -1, '设置文本块')
    ...
    def WorkStation(self):
        #最右侧的综述栏
        #上面的自动生成的、不可编辑的综述
        text1 = wx.StaticBox(self.panel, -1, '编辑综述')
    ...
    def ShowBlock(self, e):
        ...
        self.AddStat(('子话题：%s 有 %d 个文本块.')%(self.topics[curid].label, self.k))
    ...
    def search(self):
        ...
        self.AddStat(('当前话题: %s 正在被搜索...')%(self.newsname))
    ...
    def summary(self):
        ...
        if selecnt == 0:
            self.AddStat('没有子话题被选中')
            return
        elif selecnt == 1:
            self.AddStat('1个子话题被选中了.')
        else:
            self.AddStat(('%d个子话题被选中了.')%(selecnt))
    ...
    def OnSave(self, e):
        dlg = wx.FileDialog(self, defaultFile = self.newsname+'_synthesi.txt',
         style = wx.FD_SAVE, name = '保存综述')
        if dlg.ShowModal() == wx.ID_OK:
            f = codecs.open(dlg.GetPath(), 'w', 'utf8')
            f.write(self.solidsum.GetValue())
            f.close()
            self.AddStat(('%s 的综述已被保存.')%(self.newsname))
        dlg.Destroy()
    ...
    def OnAbout(self, e):
        helps = '输入话题名称，系统会爬取若干篇新闻，从中抽取出若干词组作为子话题'
        helps += '用户可以选择若干子话题，生成相应的综述。'
